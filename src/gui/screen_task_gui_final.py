...
# Mejoras: Pestañas, Audio WASAPI Loopback, Favicon, Sincronizacion A/V
...
class AudioCaptureWASAPI:
    ...
    def __init__(self, device_index=None, sample_rate=44100, channels=2, chunk_size=1024):
        ...
        self.running = False
        self.audio_queue = deque(maxlen=200)  # Buffer circular para sincronizacion
        self.stream = None
        self.pa = None
    ...
    def get_wasapi_devices(self):
        ...
        try:
            ...
            for i in range(self.pa.get_device_count()):
                ...
                # Buscar dispositivos de salida (speakers) que soporten WASAPI loopback
                if info.get('hostApi') == 3:  # WASAPI host API index
                    # Verificar si es dispositivo de salida (maxOutputChannels > 0)
    ...
    def start(self, device_index=None):
        ...
        try:
            ...
            # Abrir stream en modo loopback (captura salida del sistema)
...
class ScreenStreamHandler(BaseHTTPRequestHandler):
    ...
    def send_stream(self):
        ...
        # Si hay audio activo (modo VLC), enviar stream con audio
        if s.audio_enabled and s.audio_capture and s.audio_capture.running:
            self.send_audio_video_stream()
        else:
    ...
    def send_audio_video_stream(self):
        """Stream MJPEG con audio sincronizado (modo VLC)"""
        self.send_response(200)
        self.send_header('Content-type', 'multipart/x-mixed-replace; boundary=frame')
        self.send_header('Cache-Control', 'no-cache')
        self.end_headers()
        ...
        # Buffer de audio para sincronizacion
        audio_buffer = b''
        audio_sample_size = s.audio_capture.channels * 2  # 16-bit = 2 bytes por muestra
        audio_samples_per_frame = int(s.audio_capture.sample_rate * (s.ms_interval / 1000.0))
        audio_bytes_per_frame = audio_samples_per_frame * audio_sample_size

        try:
            while s and s.running:
                try:
                    # Capturar pantalla
                    screenshot = s.screen_capture.capture()
                    img_byte_arr = io.BytesIO()
                    screenshot.save(img_byte_arr, format='JPEG', quality=s.quality, optimize=True)
                    img_byte_arr = img_byte_arr.getvalue()

                    # Acumular audio del buffer
                    while len(audio_buffer) < audio_bytes_per_frame:
                        audio_chunk = s.audio_capture.get_audio_frame()
                        if audio_chunk:
                            audio_buffer += audio_chunk
                        else:
                            break

                    # Tomar solo los bytes necesarios para sincronizar
                    if len(audio_buffer) >= audio_bytes_per_frame:
                        frame_audio = audio_buffer[:audio_bytes_per_frame]
                        audio_buffer = audio_buffer[audio_bytes_per_frame:]
                    else:
                        frame_audio = audio_buffer + b'\x00' * (audio_bytes_per_frame - len(audio_buffer))
                        audio_buffer = b''

                    # Enviar frame con metadata de audio
                    metadata = {
                        'audio_present': True,
                        'audio_rate': s.audio_capture.sample_rate,
                        'audio_channels': s.audio_capture.channels,
                        'audio_size': len(frame_audio)
                    }

                    frame_header = b'--frame\r\n'
                    frame_header += b'Content-Type: image/jpeg\r\n'
                    frame_header += f'Content-Length: {len(img_byte_arr)}\r\n'.encode('utf-8')
                    frame_header += f'X-Audio-Rate: {metadata["audio_rate"]}\r\n'.encode('utf-8')
                    frame_header += f'X-Audio-Channels: {metadata["audio_channels"]}\r\n'.encode('utf-8')
                    frame_header += f'X-Audio-Size: {metadata["audio_size"]}\r\n'.encode('utf-8')
                    frame_header += b'\r\n'
                    ...
                    time.sleep(s.ms_interval / 1000.0)

                except Exception as e:
                    s.log(f"Error stream A/V: {e}")
                    time.sleep(0.5)

        except (BrokenPipeError, ConnectionResetError, ConnectionAbortedError):
            pass
        except Exception as e:
            s.log(f"Error stream: {e}")
        finally:
...
class ScreenTaskServer:
    ...
    def get_audio_devices(self):
        """Obtiene dispositivos de audio WASAPI disponibles"""
        if not PYAUDIO_AVAILABLE:
            return []
        try:
    ...
    def start(self, port, quality, fps, ms_interval, private, username, password, 
        ...
        if audio_enabled and vlc_mode and PYAUDIO_AVAILABLE and audio_device_index is not None:
...
.audio-badge {{ background: #e94560; color: white; padding: 5px 15px; border-radius: 20px; font-size: 0.9rem; display: inline-block; margin: 10px 0; }}
</style></head>
<body>
...
<div class="url-box">
    ...
    {'<div class="audio-badge">&#127911; Audio del sistema incluido</div>' if self.audio_enabled else ''}
</div>
<div class="steps">
...
class ScreenTaskGUI:
    ...
    def build_ui(self):
        ...
        self.client_label = tk.Label(self.root, text="Clientes conectados: 0", 
        ...
        audio_info = tk.Label(tab3, text="Audio solo disponible en modo VLC (Transmitir a VLC)", 
        ...
        if not PYAUDIO_AVAILABLE:
            ...
            devices_frame = tk.LabelFrame(tab3, text="Dispositivos de Audio WASAPI", bg="#2d2d2d", 
                                         fg="#e94560", font=('Segoe UI', 10, 'bold'), padx=10, pady=10)
            ...
            device_names = [f"{d['index']}: {d['name']}" for d in audio_devices] if audio_devices else ["No se encontraron dispositivos WASAPI"]
            ...
            tk.Label(options_frame, text="Frecuencia de muestreo:", bg="#2d2d2d", fg="#ffffff", 
                    font=('Segoe UI', 10)).grid(row=1, column=0, sticky=tk.W, pady=5)
            self.sample_rate_combo = ttk.Combobox(options_frame, 
                values=["44100 Hz (CD)", "48000 Hz (DVD)", "96000 Hz (HD)"], 
                state="readonly", width=20)
            self.sample_rate_combo.grid(row=1, column=1, sticky=tk.W, padx=5, pady=5)
            self.sample_rate_combo.current(0)

            # Info
            info_text = """Notas importantes:
...
- El audio se sincroniza automaticamente con el video en el stream
- VLC puede reproducir el stream con audio incluido"""
    ...
    def get_sample_rate(self):
        """Obtiene la frecuencia de muestreo seleccionada"""
        if not hasattr(self, 'sample_rate_combo'):
            return 44100
        selected = self.sample_rate_combo.get()
        if "44100" in selected:
            return 44100
        elif "48000" in selected:
            return 48000
        elif "96000" in selected:
            return 96000
        return 44100
        ...
        try:
            ...
            if self.server.start(port, quality, fps, ms_interval, private, username, password, 
                ...
                if audio_enabled and vlc_mode:
                    ...
                    self.log(f"    Frecuencia: {self.get_sample_rate()} Hz")
                    self.log("[!] Nota: El audio se sincroniza automaticamente con el video")
